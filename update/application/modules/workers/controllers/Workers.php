    ...
    // = $this->weighing_model->get_nutritions($postdata)){
              ...
              foreach ($result as $key => $value) {
               ...
               $data[] = array(
                  ...
                  $value->hfa,
                  $value->wfh,
                  '<a class="btn btn-info btn-xs" href="'.site_url('students/profile/').$value->student_id.'" type="button"><i class="fas fa-list"></i></a> <button class="btn btn-danger btn-xs btn-trash nutritions" data-id="'.$value->weighing_id.'" type="button"><i class="fas fa-trash"></i></button>'
                ); 
               $i++;
              }
      echo json_encode(array('data'=>$data));
