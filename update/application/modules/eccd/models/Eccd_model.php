...
/**
 ...
 class Eccd_model extends CI_Model
 {
	...
	public function listbyyear($table,$year_id,$center_id=0,$worker_id = 0 ,$student_id = 0)
	{
		...
		$sql = sprintf("SELECT t1.*,w.date_weighing,w.weight,w.height, w.wfa, w.hfa, w.wfh, w.status FROM `".$table."` `t1` LEFT JOIN e_weighing w ON w.id = t1.weighing_id WHERE center_id = %u and worker_id = %u and student_id = %u and year_id = %u",$center_id,$worker_id,$student_id,$year_id);

		$query = $this->db->query($sql);
		return $query->result();
		}elseif($center_id && $worker_id){

		$sql = sprintf("SELECT t1.*,w.date_weighing,w.weight,w.height, w.wfa, w.hfa, w.wfh, w.status FROM `".$table."` `t1` LEFT JOIN e_weighing w ON w.id = t1.weighing_id WHERE center_id = %u and worker_id = %u and year_id = %u",$center_id,$worker_id , $year_id);
		$query = $this->db->query($sql);
		return $query->result();
		...
		$sql = sprintf("SELECT t1.*,w.date_weighing,w.weight,w.height, w.wfa, w.hfa, w.wfh, w.status FROM `".$table."` `t1` LEFT JOIN e_weighing w ON w.id = t1.weighing_id WHERE center_id = %u and year_id = %u",$center_id,$year_id);
		$query = $this->db->query($sql);
		return $query->result();
		...
		$sql = sprintf("SELECT t1.*,w.date_weighing,w.weight,w.height, w.wfa, w.hfa, w.wfh, w.status FROM `".$table."` `t1` LEFT JOIN e_weighing w ON w.id = t1.weighing_id WHERE  year_id = %u ",$year_id);
				$query = $this->db->query($sql);
				return $query->result();
	...
	public function listall($table,$center_id=0,$worker_id = 0 ,$student_id = 0)
	{
		...
		$sql = sprintf("SELECT t1.*,w.date_weighing,w.weight,w.height, w.wfa, w.hfa, w.wfh, w.status FROM `".$table."` `t1` LEFT JOIN e_weighing w ON w.id = t1.weighing_id WHERE center_id = %u and worker_id = %u and student_id = %u",$center_id,$worker_id,$student_id);

		$query = $this->db->query($sql);
		return $query->result();
		}elseif($center_id && $worker_id){

		$sql = sprintf("SELECT t1.*,w.date_weighing,w.weight,w.height, w.wfa, w.hfa, w.wfh, w.status FROM `".$table."` `t1` LEFT JOIN e_weighing w ON w.id = t1.weighing_id WHERE center_id = %u and worker_id = %u",$center_id,$worker_id);
		$query = $this->db->query($sql);
		return $query->result();
		...
		$sql = sprintf("SELECT t1.*,w.date_weighing,w.weight,w.height, w.wfa, w.hfa, w.wfh, w.status FROM `".$table."` `t1` LEFT JOIN e_weighing w ON w.id = t1.weighing_id WHERE center_id = %u",$center_id);
		$query = $this->db->query($sql);
		return $query->result();
		...
		$sql = sprintf("SELECT t1.*,w.date_weighing,w.weight,w.height, w.wfa, w.hfa, w.wfh, w.status FROM `".$table."` `t1` LEFT JOIN e_weighing w ON w.id = t1.weighing_id;");
				$query = $this->db->query($sql);
				return $query->result();
