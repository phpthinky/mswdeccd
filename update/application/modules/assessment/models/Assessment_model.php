...
public function get_raw_score($student_id=0)
  {
    // code...
/*
    $sql =sprintf("SELECT t.student_id,t.date_assessment,t.domain,
...
(SELECT t1.raw_score FROM `assessment` `t2` WHERE t.student_id = t1.student_id and t1.domain = t.domain AND t2.type = 2 LIMIT 1) as raw_score_2,
...
FROM `assessment` `t` WHERE t.student_id = %u GROUP BY t.domain;",$student_id);
