    ...
    $(document).on('click','.tomodal_sum_scaled_score',function(){
    	//console.log($(this).attr())
    	var student_id = $(this).data('id')
    	var score = $(this).data('score')
    	var year_id = $('#assessment-classess').val();
    	var type = $('#ass_type').val();
    	//alert(student_id)
    	var url = '<?=site_url()?>/assessment/list_by_domain/'+worker_id+'/'+year_id+'/'+type+'/'+score;
    	$.getJSON(url,function(response){
    		console.log(response)
			var expression = new RegExp(student_id);

			$.each(response.data,function(key,value){

				if (value.student_id == student_id) {
				console.log(expression)
				$('.label-student-name').text(value.student_name)
					$('td.td-ass-gross_motor').text(value.gross_motor)
					$('td.td-ass-fine_motor').text(value.fine_motor)
					$('td.td-ass-self_help').text(value.self_help)
					$('td.td-ass-expressive_lang').text(value.expressive_lang)
					$('td.td-ass-receiptive_lang').text(value.receiptive_lang)
					$('td.td-ass-cognitive').text(value.cognitive)
					$('td.td-ass-social_emotion').text(value.social_emotion)
					return false;
				}
			})

    	})
    	$('#modal_sum_scaled_score').modal('show')
    })
